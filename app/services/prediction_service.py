...
class PredictionService:
    ...
    async def predict_batch(self, email_bodies: List[str]) -> List[Dict[str, Any]]:
        ...
        for email_body in email_bodies:
            try:
                ...
                # Add error result
                predictions.append(
                    {"is_interview": "No", "confidence": 0.0, "error": str(e)}
                )
    ...
    def cross_validate_model(self, X, y, cv: int = 5) -> Dict[str, Any]:
        ...
        cv_scores = cross_val_score(
            self.model_manager.model, X, y, cv=cv, scoring="accuracy"
        )
    ...
    def get_prediction_explanation(
        self, email_body: str, prediction: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        return {
            ...
            "confidence_level": (
                "high" if self.is_high_confidence_prediction(confidence) else "low"
            ),
            "key_features": {"keywords": keywords, "text_features": features},
        }
