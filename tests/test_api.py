...
class TestAPIEndpoints:
    ...
    def test_predict_endpoint_no_model(self, client):
        ...
        response = client.post("/v1/predict", json={"email_body": "Test email content"})
        assert response.status_code == 503

    def test_predict_endpoint_invalid_request(self, client):
        """Test prediction endpoint with invalid request."""
        # Missing email_body - should return 503 (model not loaded) or 422 (validation error)
        response = client.post("/v1/predict", json={})
        assert response.status_code in [422, 503]

        # Empty email_body - should return 503 (model not loaded) or 400 (validation error)
        response = client.post("/v1/predict", json={"email_body": ""})
        assert response.status_code in [400, 503]

    def test_predict_endpoint_valid_request(self, client, mock_model_manager):
        """Test prediction endpoint with valid request."""
        # Since model is not loaded, expect 503
        response = client.post(
            "/v1/predict", json={"email_body": "We'd like to schedule an interview"}
        )
        assert response.status_code == 503
    ...
    def test_rate_limiting(self, client):
        ...
        for _ in range(5):
            response = client.post("/v1/predict", json={"email_body": "Test email"})
