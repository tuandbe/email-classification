...
def evaluate_model(model, X_test: np.ndarray, y_test: np.ndarray) -> dict:
    ...
    f1_interview = 2 * (precision_interview * recall_interview) / (precision_interview + recall_interview) if (precision_interview + recall_interview) > 0 else 0
    
    precision_non_interview = tn / (tn + fn) if (tn + fn) > 0 else 0
    recall_non_interview = tn / (tn + fp) if (tn + fp) > 0 else 0
    f1_non_interview = 2 * (precision_non_interview * recall_non_interview) / (precision_non_interview + recall_non_interview) if (precision_non_interview + recall_non_interview) > 0 else 0
    ...
    logger.info("Model evaluation completed", accuracy=accuracy, f1_interview=f1_interview)
    return metrics
...
def save_model(model, vectorizer: TfidfVectorizer, metrics: dict, cv_results: dict, training_data_info: dict) -> None:
    """
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Train email interview classification model")
    parser.add_argument("csv_path", type=str, help="Path to training CSV file")
    parser.add_argument("--test-size", type=float, default=0.2, help="Test set size (default: 0.2)")
    parser.add_argument("--random-state", type=int, default=42, help="Random state for reproducibility")
    parser.add_argument("--use-smote", action="store_true", help="Use SMOTE for oversampling")
    ...
    try:
        ...
        logger.info("Data split completed", train_size=len(X_train), test_size=len(X_test))
        ...
        logger.info("Text vectorization completed", features=X_train_vectorized.shape[1])
        ...
        if args.use_smote:
            ...
            logger.info("SMOTE oversampling completed", new_train_size=len(X_train_vectorized))
        ...
        print(f"  Cross-validation Score: {cv_results['mean_cv_score']:.3f} ± {cv_results['std_cv_score']:.3f}")
        print("="*50)
