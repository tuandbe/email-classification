...
async def test_model_info_endpoint():
    ...
    async with httpx.AsyncClient() as client:
        try:
            response = await client.get(f"http://localhost:{settings.port}/v1/model-info")
            print(f"Status: {response.status_code}")
            print(f"Response: {json.dumps(response.json(), indent=2)}")
            return response.status_code == 200
        except Exception as e:
...
async def main():
    ...
    if not health_ok:
        ...
        print(f"Start the server with: python -m uvicorn app.main:app --host {settings.host} --port {settings.port}")
        return
